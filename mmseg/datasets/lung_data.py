...
class LungDataset(CustomDataset):
        ...
        if split is not None:
            with open(split) as f:
                for line in f:
                    ...
                    if mmcv.is_list_of(img_suffixes, str):
                        ...
                        img_name = osp.join(img_dir, '{}{}'.format(img_id, img_suffixes))
                        img_info['filename'] = img_name
                        if ann_dir is not None:
                            seg_map = osp.join(self.ann_dir, "{}{}".format(img_id, seg_map_suffix))
            ...
            for img_id in img_ids:
                ...
                if mmcv.is_list_of(img_suffixes, str):
                    ...
                    if img_suffixes not in img_id:
                        continue
                    img_id = img_id[:img_id.find(img_suffixes)]
                    img_name = osp.join(self.img_dir, '{}{}'.format(img_id, img_suffixes))
                    # print("[DEBUG] img_name: {}".format(img_name))
                    img_info['filename'] = img_name
                    if ann_dir is not None:
                        seg_map = osp.join(self.ann_dir, "{}{}".format(img_id, seg_map_suffix))
                        img_info["ann"] = dict(seg_map=seg_map)
    ...
    def evaluate(self,
        ...
        gt_seg_maps = expand_as_one_hot(gt_seg_maps, results.size(1),
                                        kwargs.get("background_as_first_channel", True))
