...
class BaseDecodeHead3D(nn.Module, metaclass=ABCMeta):
    """Base class for BaseDecodeHead.
    ...
    def __init__(self,
                 ...
                 *,
                 ...
                 sampler=None,
                 align_corners=False):
        super(BaseDecodeHead3D, self).__init__()
        self._init_inputs(in_channels, in_index, input_transform)
        self.channels = channels
        self.num_classes = num_classes
        self.dropout_ratio = dropout_ratio
        ...
        if mmcv.is_list_of(loss_cfg, dict):
            ...
            for i in range(len(loss_cfg)):
                self.loss_decode.append(build_loss(loss_cfg[i]))
        else:
            self.loss_decode = build_loss(loss_cfg)
            ...
            for i in range(len(self.loss_decode)):
                ...
                if isinstance(l, dict):
                    for key, val in l.items():
                        loss["{}_{}".format(self.loss_decode[i].__class__.__name__, key)] = val
                else:
                    loss["loss_{}".format(self.loss_decode[i].__class__.__name__)] = l
        else:
