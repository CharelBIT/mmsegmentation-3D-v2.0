...
class EncoderDecoder(BaseSegmentor):
    ...
    def _init_decode_head(self, decode_head):
        ...
        if hasattr(self.decode_head, '_use_sigmoid'):
            self._use_sigmoid = self.decode_head._use_sigmoid
        else:
            self._use_sigmoid = False

        if hasattr(self.decode_head, '_background_as_first_channel'):
            self._background_as_first_channel = self.decode_head._background_as_first_channel
        else:
            self._background_as_first_channel = True
    ...
    def encode_decode_3d(self, img, img_metas):
        """Encode images with backbone and decode into a semantic segmentation
        map of the same size as input."""
        x = self.extract_feat(img)
        out = self._decode_head_forward_test(x, img_metas)
        ...
        for x_idx in range(x_grids):
            for y_idx in range(y_grids):
                for z_idx in range(z_grids):
                    ...
                    crop_seg_logit = self.encode_decode_3d(crop_img, img_meta)# n x c x x x y x z
                    # preds += F.pad(crop_seg_logit,
